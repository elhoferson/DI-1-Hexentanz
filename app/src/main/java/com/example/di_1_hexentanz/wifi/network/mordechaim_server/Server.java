...
public class Server {
	...
	public Server(int port, int messageHandlingThreadCount) {
		this.port = port;
		clients = Collections.synchronizedMap(new HashMap<Integer, ConnectionToClient>());
	 ...
	 * TIMEOUT milliseconds on a read. We strongly advise not to
	...
	public boolean start() {
		...
		try {
			...
			Log.e(TAG, "error opening server", e);
			return false;
		}
		...
		public void run() {
			while (running() && !serverSocket.isClosed()) {
				...
				try {
					...
					Log.e(TAG, "you did something wrong", rte);
			...
			Thread authentication = new Thread("Authentication thread") {
				public void run() {
					...
					try {
						...
						Log.e(TAG, "socket timeout interrupted", e);
						return;
					}
